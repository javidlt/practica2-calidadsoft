#!/usr/bin/env python3
"""
File2.py - Model Downloading and Management Module
...
class ModelDownloader:
    ...
    def _is_model_available_online(self, model_name: str) -> bool:
        ...
        common_models = {
            "bert-base-uncased", "gpt2", "distilbert-base-uncased",
            "roberta-base", "t5-small", "xlnet-base-cased"
        }
...
class ModelManager:
    ...
    def get_models_by_library(self, library: str) -> List[ModelInfo]:
        ...
        """
        return [model for model in self.models.values() 
                if model.library == library.lower()]
    ...
    def _cleanup_groups(self):
        ...
        empty_groups = [group for group, models in self.model_groups.items() 
...
def search_models(manager: ModelManager, query: str) -> List[ModelInfo]:
    ...
    for model in manager.get_all_models():
        if (query_lower in model.name.lower() or 
            query_lower in model.task_type.lower() or
            query_lower in model.library.lower()):
